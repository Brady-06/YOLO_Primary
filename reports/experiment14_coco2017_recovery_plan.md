# 实验14–15：COCO2017模型谱系、Taylor剪枝与恢复诊断
...
服务器环境为A800 80 GB、Python 3.12.11、PyTorch 2.13.0+cu132、
Ultralytics 8.4.144、Torch-Pruning 1.6.0。实验15使用官方YOLO11s
（SHA256 `85a76fe8...d502d5`）作为唯一父权重，在固定的2048张
train2017校准集上计算Taylor分数；val2017只用于父模型和最终模型评估。

| 阶段 | 参数量 | GMACs | mAP50-95 | 相对官方模型 |
|---|---:|---:|---:|---:|
| 官方YOLO11s | 9,458,752 | 10.7991 | 0.4633 | 0.0000 |
| Taylor结构化剪枝raw | 8,516,768 | 9.7159 | 0.1723 | -0.2910 |
| Taylor剪枝后恢复10轮 | 8,516,768 | 9.7159 | 0.4144 | -0.0488 |

Taylor流程执行43个剪枝步骤，最终GMAC减少10.03%，参数减少约9.96%。
原始剪枝造成很大精度下降，说明当前逐步评分和10%预算仍过于激进；恢复训练
追回0.2421 mAP50-95，但未回到官方父模型精度。

同一`standard_recovery`设置也训练了未剪枝父模型。父模型训练所得best为
0.4206，低于输入权重0.4633，因此最终选择器保留了source。这个对照表明
`lr0=1e-3`的10轮恢复设置会显著扰动成熟的COCO预训练权重。剪枝模型恢复后
与同条件父模型best只差0.0062，但这不能消除它相对官方输入仍有0.0488的差距。

本轮结论是：结构化剪枝和恢复链路已经跑通，谱系、哈希和数据隔离也通过；
当前权重还不能作为理想部署结果。下一轮应先固定本次剪枝结构，使用更低学习率
（优先`1e-4`，正常更新BN）做恢复对照；若仍不能明显超过0.4144，再把正式
预算降到5% GMAC，而不是立即叠加蒸馏或更换评分方法。

本地完整产物位置：

- `runs/prune/experiment15_taylor_coco2017/20260915_020645/`
- `runs/diagnostics/experiment14_coco2017_recovery/20260915_022010/`

关键权重SHA256：

- raw剪枝：`9fba0cd61fe5fd46ae2286153f8ef65205bcb309f7baec6f9f1d61fe7f1d1f0b`
- 恢复best：`7948404265b3034f61315e8e82ec5ad8b53a8b51a9737bccb73e7f57143ee356`
- 恢复last：`625ff2083f0bd066edf3ed618c846f4db4c299fd3c88a328e9d0ce265bc3bb79`
