...
- 官方YOLO11s上的COCO2017 Taylor结构化剪枝与同条件恢复对照；
- 剪枝 YOLO11n 的 P4 单尺度 DINOv2 蒸馏；
- P3/P4/P5 多尺度 DINOv2 蒸馏；
- 实验报告、运行记录与学习笔记整理。

COCO128学习主线仍以实验05贪心剪枝`best.pt`为学生模型。COCO2017正式主线已完成10.03% GMAC的Taylor剪枝与10轮恢复；恢复模型达到0.4144 mAP50-95，但仍低于官方YOLO11s的0.4633，因此暂不作为最终部署模型。
...
| 5 | COCO2017 + YOLO11s | 14–15 | 模型谱系核对、正式Taylor剪枝与恢复诊断 |

实验 11 是结构化剪枝方法与代码速查，并包含 GroupNorm 等探索，不作为统一口径的正式性能结论。

实验14–15已在A800服务器完成正式运行。计划、基线哈希、实测结果和结论见
...
正式结果：参数减少约9.96%，GMAC减少10.03%；raw剪枝mAP50-95为
0.1723，恢复10轮后为0.4144。同设置训练的未剪枝父模型best为0.4206，
而官方输入权重为0.4633，说明当前`lr0=1e-3`恢复设置本身过强。下一步先在
相同结构上验证更低学习率和正常BN更新，再决定是否把剪枝预算降到5%。
